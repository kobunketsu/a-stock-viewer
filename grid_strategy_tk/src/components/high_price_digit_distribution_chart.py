...
class HighPriceDigitDistributionChart:
    """显示最高价末位数字（分）分布的柱状图组件"""
    def __init__(self, parent_frame):
        ...
        self.title_label = ttk.Label(
            self.title_frame,
            text=l("high_price_digit_distribution"),  # "最高价末位数字(分)分布"
            font=('SimHei', 9)
        )
    ...
    def update_data(self, distribution: pd.Series, start_date: str = None, end_date: str = None):
        ...
        bars = self.ax.bar(digits, probabilities, color='pink')  # 使用不同的颜色区分最高价
    ...
    def show_loading(self, message=l("loading")+"..."):
        """显示加载信息"""
        self._initialize_chart()
    ...
    def show_error(self, message=l("error_loading_data")):
        """显示错误信息"""
        self._initialize_chart()
        self.ax.text(0.5, 0.5, message,
                     horizontalalignment='center',
